...
// setupElement ();
  ...
  background(0);
   ...
   //   setUIButtonsPosition(); // pannel b
    //  elementData.get(60).plug_page();
       circuit_position = loadImage("pcb_memorypositions_transp.gif");
       
 //       elementData.get(4).setDisplay(true);
...
loadTableSettings("Default_preset.csv");
 ...
 //{ del_.show();   }
// if (elementData.get(i).toggleMode == 26)  { if ( edit_mode == true) del_.hide(); else del_.show();}
...
public void sawplay ()
{
  if ( soundstatus == true && sawplay == false) {saw.stop(); sawplay = true;}
  if ( soundstatus == false && sawplay == true) {saw.play(); sawplay = false;}
}
...
{  
  ...
  if (raw1 < 160) {saw.freq(frequenza2(Note)); saw.amp(PApplet.parseInt(PApplet.parseBoolean(Intensity)));}
    else //  if (raw1 < 176)
  { saw.freq(frequenza2(Intensity)); saw.amp(0.5f); feedback_counter = 0; }
   // monitor_flux6 [0] = raw1; monitor_flux6 [1] = Note ; monitor_flux6 [2] = Intensity;
}
} 
...
//  initMidi();
 ...
 //   myBus.addOutput(DartOUT) ; 
  ...
  if (edit.isPressed()) {if (edit.isOn() == true ) {edit_activate(); // saw.play(); 
